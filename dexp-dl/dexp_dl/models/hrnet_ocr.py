...
from dexp_dl.models.initializer import XavierGluon
from dexp_dl.models.ocr import SpatialGather_Module, SpatialOCR_Module
from dexp_dl.models.resnetv1b import BasicBlockV1b, BottleneckV1b
...
class HighResolutionNet(nn.Module):
    def __init__(
        ...
        self.conv1 = self.conv_layer(
            num_channels, 64, kernel_size=3, stride=2, padding=1, bias=False
        )
        self.bn1 = norm_layer(64)

        self.conv2 = self.conv_layer(
            64, 64, kernel_size=3, stride=second_stride, padding=1, bias=False
        )
    ...
    def forward(self, x, additional_features=None):
        ...
        if self.ocr_width > 0:
            ...
            out_aux = interpolate(out_aux, size=x.shape[2:], mode="linear")
            feats = self.ocr_distri_head(feats, context)
            out = self.cls_head(feats)
            out = interpolate(out, size=x.shape[2:], mode="linear")
            return [out, out_aux]
        else:
            return [interpolate(self.cls_head(feats), size=x.shape[2:], mode="linear")]
    ...
    def aggregate_hrnet_features(self, x):
        # Upsampling
        x1 = interpolate(
            x[1], size=x[0].shape[2:], mode="linear", align_corners=self.align_corners
        )
        x2 = interpolate(
            x[2], size=x[0].shape[2:], mode="linear", align_corners=self.align_corners
        )
        x3 = interpolate(
            x[3], size=x[0].shape[2:], mode="linear", align_corners=self.align_corners
        )
...
def hrnet(pretrained_path: str = "", **kwargs) -> nn.Module:
...
def hrnet_18_ocr_64(**kwargs) -> nn.Module:
    return hrnet(width=18, ocr_width=64, small=False, **kwargs)


def hrnet_18_ocr_48_small(**kwargs) -> nn.Module:
    return hrnet(width=18, ocr_width=48, small=True, **kwargs)


def hrnet_18_small(**kwargs) -> nn.Module:
    return hrnet(width=18, ocr_width=0, small=True, **kwargs)
