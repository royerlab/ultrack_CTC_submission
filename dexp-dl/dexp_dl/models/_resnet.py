    ...
    if stride == 1 and dilation == 1:
        ...
        if not isinstance(conv_layer, nn.Conv2d):
            raise NotImplementedError(
                "Not implemented for convolution different than 2D."
            )
        avg_pool_fn = (
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        elif isinstance(conv_layer, nn.Conv3d):
        ...
        if replace_stem_pool:
            ...
            if aa_layer is not None:
                self.maxpool = nn.Sequential(
                    *[
                        maxpool_layer(kernel_size=3, stride=1, padding=1),
                        aa_layer(channels=inplanes, stride=2),
                    ]
                )
            else:
                self.maxpool = maxpool_layer(kernel_size=3, stride=2, padding=1)
