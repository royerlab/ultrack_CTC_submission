...
from cucim.skimage.util import img_as_float, img_as_uint
...
def equalize_adapthist(image, kernel_size=None, clip_limit=0.01, nbins=256):
    ...
    * For RGBA images, the original alpha channel is removed.
    ...
    image = cp.around(rescale_intensity(image, out_range=(0, NR_OF_GRAY - 1))).astype(
        cp.uint16
    )
...
def _clahe(image, kernel_size, clip_limit, nbins):
    ...
    pad_end_per_dim = [
        (k - s % k) % k + math.ceil(k / 2.0) for k, s in zip(kernel_size, image.shape)
    ]
    ...
    hist_blocks_axis_order = tuple(range(0, ndim * 2, 2)) + tuple(range(1, ndim * 2, 2))
    ...
    map_array = cp.pad(hist, [(1, 1) for _ in range(ndim)] + [(0, 0)], mode="edge")
    ...
    blocks = blocks.reshape((_misc.prod(ns_proc), _misc.prod(blocks.shape[ndim:])))
...
@click.command()
@click.option("--input-dataset", "-i", required=True, type=str)
@click.option("--output-dataset", "-o", required=True, type=str)
@click.option("--channel", "-c", required=True, type=str)
@click.option(
    "--clip-limit",
    "-cl",
    type=float,
...
@click.option(
    ...
    type=float,
    default=None,
    help="Block size for local histogram computationg.",
)
@click.option(
    "--overwrite",
    "-w",
    ...
    type=bool,
    default=False,
    help="Overwrites output dataset if necessary",
)
def cli_clahe(
    input_dataset: str,
    output_dataset: str,
    ...
    in_ds = ZDataset(input_dataset)
    mode = "w" if overwrite else "w-"
    out_ds = ZDataset(output_dataset, mode=mode, parent=in_ds)

    dtype = np.float16
    in_arr = in_ds.get_array(channel)
    out_arr = out_ds.add_channel(channel, shape=in_arr.shape, dtype=dtype)

    if block_size is not None:
        block_size = _parse_chunks(block_size)
    else:
        block_size = list(in_arr.shape[-3:])
        block_size[0] /= 20
        print(f"Using block size of {block_size}")

    for t in tqdm(range(in_arr.shape[0])):
        out = equalize_adapthist(
            cp.asarray(in_arr[t]), kernel_size=block_size, clip_limit=clip_limit
        )
        out_arr[t] = out.get()
